    ...
    public partial class access : Form
    {
        ...
        private void access_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                string sql = "select * from Accessory_Table ORDER BY Accessory_ID DESC;";
                connection.Open();
                SqlCommand command = new SqlCommand(sql, connection);
                SqlDataReader show = command.ExecuteReader(CommandBehavior.SingleResult);
                while (show.Read())
                {
                    string id = show["Accessory_ID"].ToString();
                    string price = show["Price"].ToString();
                    byte[] image = (byte[])(show["Image"]);
                    string name = show["Accessory_Name"].ToString();
                    string got = "accessory";
